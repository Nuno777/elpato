                        ...
                        @foreach ($users as $user)
                            <tr>
                                ...
                                @if (auth()->check() && auth()->user()->type == 'admin')
                                    ...
                                    <td style="width: 5%" class="sorting_disabled">
                                        <!-- Botão para abrir o modal de verificação -->
                                        <button type="button" class="btn btn-dark"
                                            onclick="requestVerificationCode('{{ $user->slug }}')">
                                            <i class="mdi mdi-key" data-toggle="tooltip"></i>
                                        </button>

                                        <!-- Modal para Verificação -->
                                        <div class="modal fade" id="verificationModal{{ $user->slug }}"
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <input type="text" id="verificationCode{{ $user->slug }}"
                                                            class="form-control" maxlength="6"
                                                            placeholder="Enter code..." />
                                                    </div>
                                                    <div id="error-message{{ $user->slug }}"
                                                        style="color: #FE5461; display: none; margin-left: 15px;">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
                                                            data-dismiss="modal">Cancel</button>
                                                        <button type="button" class="btn btn-dark"
                                                            onclick="verifyCode('{{ $user->slug }}')">Verify</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        fetch(`/send-verification-code/${slug}`, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('error-message' + slug).style.display =
                    'none'; // Esconde a mensagem de erro se o código foi enviado com sucesso
                } else {
                    document.getElementById('error-message' + slug).innerText = data
                    .message; // Exibe a mensagem de erro
                    document.getElementById('error-message' + slug).style.display =
                    'block'; // Torna a mensagem visível
                }
            })
        ...
        fetch(`/verify-code/${slug}`, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('error-message' + slug).style.display =
                    'none'; // Esconde a mensagem de erro ao verificar com sucesso
                } else {
                    document.getElementById('error-message' + slug).innerText = data
                    .message; // Exibe a mensagem de erro
                    document.getElementById('error-message' + slug).style.display =
